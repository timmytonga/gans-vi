...
def fit_extrapastsgd(gen, gen2, gen_opt, disc, disc2, disc_opt, device, n_epochs, trainset, disc_repeats, make_noise, z_dim, disc_loss_fn, gen_loss_fn, display_step, c_lambda, lr, validation_noise, checkpt=True, loadcheckptpath=None):
	# ================================================================== #
	...
	gen2_losses = []
	disc2_losses = []
	#added for extrapolation
	disc_p_temp = {}
	disc_grad = {}
	for name, p in disc.named_parameters():
		disc_grad[name] = torch.zeros_like(p)
	gen_p_temp = {}
	gen_grad = {}
	for name, p in gen.named_parameters():
		gen_grad[name] = torch.zeros_like(p)
	...
	for epoch in range(last_epoch, n_epochs):
		...
		for real, label in tqdm(trainset):  # tqdm is just a progress bar
			...
			for _ in range(disc_repeats):  # train the discriminator this many times
				...
				with torch.no_grad():
					for name, p in disc.named_parameters():
						disc_p_temp[name] = p-lr*disc_grad[name]
						disc_p[name] = p
						new_state_dict = OrderedDict({name: disc_p_temp[name]})
			...
			with torch.no_grad():
				for name, p in gen.named_parameters():
					gen_p_temp[name] = p-lr*gen_grad[name]
					gen_p[name] = p
					new_state_dict = OrderedDict({name: gen_p_temp[name]})
